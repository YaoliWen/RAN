...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        vs_stack = torch.stack(vs, dim=2) # B*512*6
        alphas_stack = torch.stack(alphas, dim=2) # B*1*6
        alphas_stack = F.softmax(alphas_stack,dim=2) # B*1*6
        #pdb.set_trace()
        alphas_part_max = alphas_stack[:,:,0:5].max(dim=2)[0] # B*1
        # alphas_part_max = alphas_stack[:,:,0:3].mean(dim=2)
        alphas_org = alphas_stack[:,:,5] # B*1
        vm = vs_stack.mul(alphas_stack).sum(2).div(alphas_stack.sum(2)) # B*512
        # pdb.set_trace()
        for i in range(len(vs)): # 6
            vs[i] = torch.cat([vs[i], vm], dim=1) # B*1024 (6*B*1024)
        vs_stack_4096 = torch.stack(vs, dim=2) # B*1024*6
        # pdb.set_trace()
        betas = []
        for index, v in enumerate(vs):
            betas.append(self.beta(v)) # B*1(6*B*1)
        betas_stack = torch.stack(betas, dim=2) # B*1*6
        ...
        # pdb.set_trace()
        output = vs_stack_4096.mul(betas_stack*alphas_stack).sum(2).div((betas_stack*alphas_stack).sum(2)) # B*1024
        output = output.view(output.size(0), -1) # B*1024
        #max, index = torch.max(betas_stack*alphas_stack)
        #pdb.set_trace()
        pred_score = self.fc(output) # B*8
